...
def run_lc_algorithm(args):
    ...
    layers = [lambda x=x: getattr(x, 'weight') for x in net.modules() if isinstance(x, nn.Conv2d) or isinstance(x, nn.ConvTranspose2d)]
    n_pars = sum(map(lambda l: l().numel(), layers))
    kappa = int(0.05 * n_pars)

    compression_tasks = {
        Param(layers, device): (AsVector, ConstraintL0Pruning(kappa=kappa), 'pruning')
    }
