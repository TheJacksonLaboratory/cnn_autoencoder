...
def resume_checkpoint(cae_model, checkpoint, gpu=True):
...
def main(args):
    ...
    criterion = setup_criteria(args)

    if args.resume is not None:
        resume_checkpoint(cae_model, args.resume, gpu=args.gpu)
    ...
    valid_loss = valid(cae_model, test_data, criterion, args)
    logger.info('Testing loss {:0.4f}'.format(valid_loss))
